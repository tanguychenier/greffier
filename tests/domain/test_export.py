...
class TestCuttingALineForAScreen:
    ...
    def test_a_long_turn_is_cut_on_words(self) -> None:
        ...
        assert all(len(ligne) <= LINE_WIDTH for ligne in lignes)
...
class TestSharingATurnBetweenBlocks:
    def test_a_long_turn_becomes_several_blocks_in_order(self) -> None:
        long = " ".join(["mot"] * 60)
        morceaux = blocks_of([said(0, 30, long)])
        assert len(morceaux) > 1
        assert [b.start for b in morceaux] == sorted(b.start for b in morceaux)

    def test_the_blocks_cover_the_turn_and_do_not_overlap(self) -> None:
        morceaux = blocks_of([said(10, 40, " ".join(["mot"] * 60))])
        assert morceaux[0].start == 10
        assert morceaux[-1].end == pytest.approx(40, abs=0.01)
        for one, other in zip(morceaux, morceaux[1:], strict=False):
    ...
    def test_a_turn_too_short_for_its_blocks_overruns_rather_than_flashes(self) -> None:
        ...
        morceaux = blocks_of([said(0, 2, " ".join(["mot"] * 60))])
        assert all(b.end - b.start >= 0.2 for b in morceaux)
        assert morceaux[-1].end > 2

    def test_the_turns_come_out_in_time_order(self) -> None:
        morceaux = blocks_of([said(9, 10, "après"), said(1, 2, "avant")])
        assert [b.text for b in morceaux] == ["avant", "après"]


class TestSubtitlesAPlayerReads:
    def test_the_shape_srt_expects(self) -> None:
        sortie = srt([said(1.5, 3.25, "D'accord.")], NAMES)
        assert sortie.startswith("1\n00:00:01,500 --> 00:00:03,250\nSophie : D'accord.")

    def test_the_blocks_are_numbered_from_one_without_a_gap(self) -> None:
        sortie = srt([said(0, 4, "un"), said(5, 9, "deux")], NAMES)
        numeros = [line for line in sortie.splitlines() if line.isdigit()]
        assert numeros == ["1", "2"]
...
class TestNamingTheSpeakerWithoutRepeatingOneself:
    def test_a_long_turn_names_its_speaker_once(self) -> None:
        ...
        sortie = srt([said(0, 30, " ".join(["mot"] * 60), voice="v2")], NAMES)
        assert sortie.count("Julien :") == 1
    ...
    def test_the_name_is_counted_in_the_width_of_its_line(self) -> None:
        ...
        sortie = srt([said(0, 9, "La recette est prête depuis lundi, il manque "
                                  "la signature du prestataire.")], NAMES)
        lignes = [
            ligne for ligne in sortie.splitlines()
            if ligne and not ligne.isdigit() and "-->" not in ligne
        ]
        assert all(len(ligne) <= LINE_WIDTH for ligne in lignes)
