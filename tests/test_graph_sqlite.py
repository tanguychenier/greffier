...
from greffier.adapters import graph_sqlite as the_graph
...
class TestWhatIsKnownAboutASubject:
    def test_the_meetings_come_back_most_recent_first(self, file):
        assert the_graph.known_about(file, "recette").meetings[0] == "2026-09-12_recette"

    def test_the_people_of_those_meetings(self, file):
        """Who usually attends is what fills « attendus » without typing it."""
        assert set(the_graph.known_about(file, "recette").people) == {"Jacques", "Sophie"}

    def test_what_was_left_open(self, file):
        assert "valider les anomalies" in the_graph.known_about(file, "recette").open_points

    def test_the_documents_that_served(self, file):
        assert "cahier.pdf" in the_graph.known_about(file, "recette").documents

    def test_a_subject_nobody_ever_met_about_is_empty(self, file):
        assert the_graph.known_about(file, "inconnu").empty

    def test_no_file_is_not_an_error(self, tmp_path):
        assert the_graph.known_about(tmp_path / "rien.sqlite3", "recette").empty
...
class TestForgettingSomebody:
    def test_the_person_and_everything_said_of_them_go(self, file):
        ...
        assert the_graph.forget_person(file, "Sophie") > 0
        assert "Sophie" not in the_graph.known_about(file, "recette").people

    def test_the_others_stay(self, file):
        the_graph.forget_person(file, "Sophie")
        assert "Jacques" in the_graph.known_about(file, "recette").people

    def test_forgetting_somebody_unknown_is_not_an_error(self, file):
        assert the_graph.forget_person(file, "Personne") == 0
...
class TestItIsAnIndexAndNotTheTruth:
    def test_it_can_be_emptied_and_built_again(self, file):
        ...
        the_graph.forget_everything(file)
        assert the_graph.known_about(file, "recette").empty
        the_graph.write(file, [Node(Kind.SUBJECT, "recette"),
                            Node(Kind.MEETING, "2026-09-12_recette")],
                     [Edge(Link.ABOUT, (Kind.MEETING, "2026-09-12_recette"),
                           (Kind.SUBJECT, "recette"))])
        assert the_graph.known_about(file, "recette").meetings

    def test_writing_the_same_fact_twice_says_it_once(self, file):
        the_graph.write(file, [Node(Kind.PERSON, "Jacques")],
                     [Edge(Link.ATTENDED, (Kind.PERSON, "Jacques"),
                           (Kind.MEETING, "2026-09-12_recette"))])
        assert the_graph.known_about(file, "recette").people.count("Jacques") == 1
