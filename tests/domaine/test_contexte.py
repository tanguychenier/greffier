...
class TestTerme:
    def test_un_sigle_porte_son_sens_pour_le_redacteur(self):
        assert Terme("OTP", "mot de passe à usage unique").gloss == (
            "OTP (mot de passe à usage unique)"
        )

    def test_sans_sens_le_terme_reste_nu(self):
        assert Terme("CASA").gloss == "CASA"
...
class TestAmorce:
    ...
    def test_les_termes_et_les_noms_y_figurent_ensemble(self):
        context = Contexte(
            termes=(Terme("CASA"),),
            intervenants=(Intervenant("Katell"),),
        )
    ...
    def test_un_terme_repete_ne_compte_qu_une_fois(self):
        prompt_seed = Contexte(termes=(Terme("OTP"), Terme("OTP"))).prompt_seed()
...
class TestFusion:
    ...
    def test_la_casse_ne_cree_pas_de_doublon(self):
        fondu = Contexte(termes=(Terme("Casa"),)).join(Contexte(termes=(Terme("CASA"),)))
        assert len(fondu.termes) == 1

    def test_les_deux_sources_se_completent(self):
        fondu = Contexte(termes=(Terme("CASA"),)).join(Contexte(termes=(Terme("OTP"),)))
    ...
    def test_fusionner_ne_modifie_aucun_des_deux(self):
        ...
        general.join(Contexte(termes=(Terme("OTP"),)))
