...
class TestCompression:
    def test_avant_le_delai_on_ne_touche_a_rien(self):
        assert Regle(compresser_apres=7).decider(3, True, False) is Geste.RIEN

    def test_apres_le_delai_on_compresse(self):
        assert Regle(compresser_apres=7).decider(8, True, False) is Geste.COMPRESSER

    def test_un_audio_deja_compresse_est_laisse(self):
        assert Regle(compresser_apres=7).decider(30, True, True) is Geste.RIEN

    def test_un_delai_nul_desactive_la_compression(self):
        assert Regle(compresser_apres=0).decider(999, True, False) is Geste.RIEN
...
class TestEffacement:
    def test_desactive_par_defaut(self):
        ...
        assert Regle().decider(9999, True, True) is Geste.RIEN

    def test_active_il_efface_au_dela_du_delai(self):
        regle = Regle(compresser_apres=7, effacer_apres=90)
        assert regle.decider(91, True, True) is Geste.EFFACER

    def test_il_l_emporte_sur_la_compression(self):
        regle = Regle(compresser_apres=7, effacer_apres=90)
        assert regle.decider(120, True, False) is Geste.EFFACER
...
class TestUneReunionNonTranscriteEstIntouchable:
    ...
    def test_jamais_compressee(self):
        assert Regle(compresser_apres=1).decider(365, False, False) is Geste.RIEN

    def test_jamais_effacee(self):
        regle = Regle(compresser_apres=7, effacer_apres=30)
        assert regle.decider(365, False, True) is Geste.RIEN
