...
class TestReadingWhatTheWriterReturned:
    def test_a_clean_array_is_read(self):
        contributions = analyser(
            '[{"texte": "Le PDF ne se régénère pas", "genre": "problème", '
            '"etat": "en discussion", "sous": ""}]'
        )
        assert len(contributions) == 1
        assert contributions[0].kind is Kind.PROBLEM
        assert contributions[0].state is Standing.UNDER_DISCUSSION

    def test_a_code_fence_is_accepted(self):
        """Le rédacteur enrobe volontiers, malgré la consigne."""
        contributions = analyser('```json\n[{"texte": "Un point"}]\n```')
        assert [a.text for a in contributions] == ["Un point"]

    def test_chatter_around_the_array_is_tolerated(self):
        contributions = analyser('Voici la liste :\n[{"texte": "Un point"}]\nVoilà.')
        assert [a.text for a in contributions] == ["Un point"]
    ...
    def test_one_broken_item_does_not_lose_the_others(self):
        contributions = analyser('[{"texte": "Bon"}, "pas un objet", {"texte": "Aussi bon"}]')
        assert [a.text for a in contributions] == ["Bon", "Aussi bon"]
...
class TestCarefulAboutTheStanding:
    ...
    def test_an_unrecognised_standing_falls_back_to_under_discussion(self):
        contributions = analyser('[{"texte": "Une idée", "etat": "peut-être"}]')
        assert contributions[0].state is Standing.UNDER_DISCUSSION

    def test_overtaken_cannot_come_from_an_extraction(self):
        """Only a person marks a lead as overtaken."""
        contributions = analyser('[{"texte": "Une piste", "etat": "dépassé"}]')
        assert contributions[0].state is Standing.UNDER_DISCUSSION

    def test_agreed_is_honoured_when_it_is_explicit(self):
        contributions = analyser('[{"texte": "Monter la recette", "etat": "acté"}]')
        assert contributions[0].state is Standing.AGREED
...
class TestWhatIsAskedOfTheWriter:
    def test_the_subject_and_the_material_are_passed_on(self):
        ...
        assert "Oasis" in writer.received
        assert "On a parlé d'Oasis longuement." in writer.received
...
class TestCompletingWithoutDuplicating:
    ...
    def test_the_guidance_says_not_to_reuse_them_wrongly(self):
        ...
        flattened = " ".join(GUIDANCE.split())
        assert "Ne le reprends que s'il s'agit vraiment du même point" in flattened
