...
class TestLeCycleQuiVautDExister:
    def test_il_demande_qui_parle_puis_nomme_la_voix_et_remercie(self):
        ...
        suite = assistant.tour([dit("c'est Michel", 104.0, 105.0)], maintenant=108.0)
        assert nommees == [("12", "Michel")]
        assert suite is not None
        assert suite.propos == "Merci, c'est noté : je mets Michel sur cette voix."
...
class TestLEchangeSePoursuit:
    ...
    def test_elle_reagit_a_la_reponse_qu_on_lui_fait(self):
        ...
        suite = assistant.tour([dit("c'est Hugo qui prend", 104.0, 106.0)],
                               maintenant=109.0)
        assert suite is not None
        assert suite.propos == "Très bien, donc c'est Hugo qui s'en occupe."
    ...
    def test_la_question_posee_est_donnee_au_modele(self):
        ...
        assistant.attente = Occasion(
            raison=Raison.APPORT, propos="Qui porte la migration ?", ne_le=100.0)
        assistant.tour([dit("Hugo", 104.0, 106.0)], maintenant=109.0)
    ...
    def test_sans_cerveau_elle_ne_poursuit_pas(self):
        ...
        assert assistant.tour([dit("Hugo", 104.0, 106.0)], maintenant=109.0) is None
