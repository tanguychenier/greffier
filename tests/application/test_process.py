...
class TestTheNamesGivenDuringTheMeeting:
    ...
    def test_a_voice_left_unnamed_takes_the_name_given_live(self):
        processing = chain()
        processing.named_live = self._named(("Kevin", 13.0, 20.0))
        assert processing.run_chain(AUDIO).names["2"] == "Kevin"
    ...
    def test_it_never_overwrites_a_name_already_found(self):
        ...
        processing = chain()
        processing.named_live = self._named(("Kevin", 0.0, 30.0))
        outcome = processing.run_chain(AUDIO)
        assert outcome.names["1"] == "Tanguy"
    ...
    def test_a_disagreement_is_reported(self):
        ...
        processing.named_live = self._named(("Kevin", 0.0, 30.0))
        outcome = processing.run_chain(AUDIO)
        assert any("Kevin" in a and "Tanguy" in a for a in outcome.warnings)
...
class TestTheVoiceBank:
    ...
    def test_a_disagreement_between_bank_and_meeting_is_flagged(self):
        ...
        vectors = {(0.0, 12.0): [1.0, 0.0, 0.0], (13.0, 20.0): [0.0, 1.0, 0.0],
                    (21.0, 28.0): [1.0, 0.0, 0.0]}
        bank = FakeBank([Person("Michel", [normalise([1.0, 0.02, 0.0])])])
        processing = chain(extractor=FakeExtractor(vectors), bank=bank)
        outcome = processing.run_chain(AUDIO)
...
class TestTheContextHeader:
    ...
    def test_the_named_participants_are_listed(self) -> None:
        ...
        header = context_header("2026-09-02_16h46_x", 600.0,
                                 names=["Paul", "Camilo"], voices_heard=2)
