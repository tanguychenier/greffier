...
class TestCeQuiNEstPasEcrit:
    def test_les_chemins_ne_sont_jamais_figes(self):
        ...
        assert "[chemins]" not in reglages.render(Config())
    ...
    def test_un_champ_non_renseigne_est_omis(self):
        ...
        rendered = reglages.render(Config(locuteurs={"personnes": None}))
    ...
    def test_le_mot_de_passe_smtp_n_a_pas_de_place_ici(self):
        ...
        assert "mot_de_passe" not in reglages.render(Config())


class TestEcriture:
    def test_le_fichier_est_ecrit_a_l_endroit_ou_la_config_est_lue(self, tmp_path, garnie):
        file = reglages.save_settings(garnie, folder=tmp_path / "ailleurs")
    ...
    def test_la_version_precedente_est_conservee(self, tmp_path):
        folder = tmp_path / "c"
        reglages.save_settings(Config(appearance={"theme": "clair"}), folder=folder)
        reglages.save_settings(Config(appearance={"theme": "sombre"}), folder=folder)
        precedent = Config.load(folder / "config.toml.precedent")
        assert precedent.appearance.theme == "clair", "l'ancien réglage doit rester récupérable"
    ...
    def test_une_ecriture_qui_echoue_ne_detruit_pas_l_existant(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(reglages, "render", rendre_casse)
