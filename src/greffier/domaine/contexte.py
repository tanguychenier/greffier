...
class Contexte:
    ...
    def fusionner(self, autre: Contexte) -> Contexte:
        ...
        gens = {i.nom.casefold(): i for i in self.intervenants}
        gens.update({i.nom.casefold(): i for i in autre.intervenants})
    ...
    def amorce(self) -> str:
        ...
        """
        mots = [t.ecriture for t in self.termes] + [i.nom for i in self.intervenants]
        retenus = _tenir(mots, AMORCE_MAXIMUM - len(_PREAMBULE) - len(" Vocabulaire : ."))
        if not retenus:
            return ""
    ...
    def ecartes(self) -> tuple[str, ...]:
        ...
        """
        mots = [t.ecriture for t in self.termes] + [i.nom for i in self.intervenants]
        retenus = set(_tenir(mots, AMORCE_MAXIMUM - len(_PREAMBULE) - len(" Vocabulaire : .")))
        return tuple(m for m in mots if m not in retenus)
    ...
    def entete(self) -> str:
        ...
        if self.vide:
            return ""
        lignes = ["[Contexte du milieu de travail]"]
        if self.termes:
            lignes.append(
            ...
            lignes += [f"- {t.glose}" for t in self.termes]
...
def _tenir(mots: list[str], place: int) -> list[str]:
    ...
    retenus: list[str] = []
    longueur = 0
    for mot in dict.fromkeys(m for m in mots if m.strip()):
        ajout = len(mot) + (2 if retenus else 0)
        if longueur + ajout > place:
            continue
        retenus.append(mot)
        longueur += ajout
    return retenus
