...
def proposer(
    ...
    """
    suffixe = fichier.suffix.casefold()

    if suffixe in SONS:
        if taille is not None and taille < TAILLE_MINIMALE_SON:
            return Proposition(
                fichier, Destin.INCONNU,
                f"son trop court pour une réunion ({taille / 1024:.0f} Ko)",
            )
        return Proposition(fichier, Destin.REUNION, "enregistrement sonore")

    if suffixe in VIDEOS:
        return Proposition(
            fichier, Destin.VIDEO,
    ...
    if suffixe in TEXTES:
        return Proposition(fichier, Destin.CONTEXTE, "texte lisible tel quel")

    if suffixe in TEXTES_OUTILLES:
        besoin = "pdftotext" if suffixe == ".pdf" else "textutil"
        return Proposition(
            fichier, Destin.CONTEXTE,
...
def resumer(propositions: list[Proposition]) -> str:
    ...
    morceaux = [
        f"{combien} {destin}{'s' if combien > 1 and destin is not Destin.CONTEXTE else ''}"
